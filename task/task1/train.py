...
if __name__ == "__main__":
	...
	# 音声ファイルの読み込み
	x_a, _ = librosa.load('task1/waves/a.wav', sr=SR)
	x_i, _ = librosa.load('task1/waves/i.wav', sr=SR)
	x_u, _ = librosa.load('task1/waves/u.wav', sr=SR)
	x_e, _ = librosa.load('task1/waves/e.wav', sr=SR)
	x_o, _ = librosa.load('task1/waves/o.wav', sr=SR)
	...
	# パラメータの保存
	np.savez('task1/parameter/mu.npz', mu_a, mu_i, mu_u, mu_e, mu_o)
	np.savez('task1/parameter/sigma.npz', sigma_a, sigma_i, sigma_u, sigma_e, sigma_o)
